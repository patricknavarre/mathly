...
const MultiplicationPractice = () => {
  ...
  const calculateProgress = () => {
    return (
      (problemsCompleted % PROBLEMS_PER_LEVEL) * (100 / PROBLEMS_PER_LEVEL)
    );
  };
  ...
  const handleHint = () => {
    if (hintsRemaining > 0 && !showHint) {
      ...
      setWorkingSteps(generateHint(problem.num1, problem.num2));
    }
  };
  ...
  useEffect(() => {
    ...
    // Add focus to the container when component mounts
    const container = document.querySelector(
      ".multiplication-practice-container"
    );
    if (container) {
      container.focus();
    }
  ...
  }, [userAnswer, hintsRemaining, showHint, handleSubmit]);

  return (
    <Container maxWidth="md" sx={{ py: 4 }}>
      <Box
        className="multiplication-practice-container"
        tabIndex={0}
        sx={{
          outline: "none",
          "&:focus": { outline: "none" },
      ...
      >
        ...
        <Box sx={{ mb: 3, display: "flex", justifyContent: "flex-end" }}>
          <Typography
            variant="h6"
            sx={{ fontFamily: "Fredoka One", color: "text.secondary" }}
          >
        ...
        >
          ...
          <AnimatePresence>
            {showStreakMultiplier && (
              <StreakMultiplier
                streak={streak + 1}
                isVisible={showStreakMultiplier}
              />
            )}
          </AnimatePresence>
